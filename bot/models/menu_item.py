...
class MenuItem(Base, TimestampMixin):
    ...
    text_en: Mapped[str | None] = mapped_column(String(100), nullable=True)
    ...
    photo_file_id: Mapped[str | None] = mapped_column(String(255), nullable=True)  # Photo file_id for text+photo messages
...
class MenuTemplate(Base, TimestampMixin):
    ...
    template_data: Mapped[str] = mapped_column(Text, nullable=False)  # JSON structure
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    
    def __repr__(self) -> str:
        return f"<MenuTemplate(id={self.id}, name={self.name!r})>"
